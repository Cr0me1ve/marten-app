...
BINDIR=marten-core$(SEP)bin
ANDROID_OUT=android$(SEP)app$(SEP)libs
IOS_OUT=ios$(SEP)Frameworks
DESKTOP_OUT=marten-core$(SEP)bin
GEO_ASSETS_DIR=assets$(SEP)core

CORE_PRODUCT_NAME=marten-core
CORE_NAME=marten-lib
LIB_NAME=marten-core
CLI_NAME=MartenCli

ifeq ($(CHANNEL),prod)
	CORE_URL=https://github.com/Cr0me1ve/marten-core/releases/download/v$(core.version)
else
	CORE_URL=https://github.com/Cr0me1ve/marten-core/releases/download/draft
endif
...
generate_kotlin_protos:
	...
	# protoc \
	# 	--proto_path=marten-core/ \
	# 	--java_out=./android/app/src/main/java/ \
	# 	--grpc-java_out=./android/app/src/main/java/ \
	# 	$(shell find marten-core/v2 marten-core/extension -name "*.proto")
	rsync -av --delete \
		--include='*/' \
		--include='*.proto' \
		--exclude='*' \
		marten-core/v2 marten-core/extension ./android/app/src/main/protos/
	# # Find .proto files and update package declarations
	# find "./android/app/src/main/java/app/marten/client/protos" -type f -name "*.java" | while read -r proto_file; do \
	#     if grep -q "^package " "$$proto_file"; then \
	#         $(SED) 's/^package \([\w\.]*\)/package app.marten.client.protos.\1/g' "$$proto_file"; \
	#     fi \
	# done

generate_go_protoc:
	make -C marten-core -f Makefile protos
	echo "SED: $(SED)"
generate_dart_protoc:
	mkdir -p lib/martencore/generated
	protoc --dart_out=grpc:lib/martencore/generated --proto_path=marten-core/  $(shell find marten-core/v2 marten-core/extension -name "*.proto") 	google/protobuf/timestamp.proto ; \
...
windows-zip-release:
	...
	mkdir -p Marten; \
	unzip -q "$$ZIP_FILE" -d Marten/; \
	rm "$$ZIP_FILE"; \
	powershell.exe -NoProfile -Command "Compress-Archive -Path 'Marten' -DestinationPath '$$FILE_NAME.zip' -Force"; \
	rm -rf Marten; \
...
linux-appimage-release:
	...
	$(BLUE)Adding StartupWMClass to marten.desktop$(DONE); \
	sed -i '/^\[Desktop Entry\]/a StartupWMClass=app.marten.client' "squashfs-root/marten.desktop"; \
	...
	ARCH=x86_64 appimagetool --no-appstream squashfs-root Marten.AppImage > /dev/null; \
	$(BLUE)Cleaning up squashfs$(DONE); \
	rm -rf squashfs-root; \
	$(YELLOW)Creating Portable Package$(DONE); \
	PKG_DIR_NAME="marten-linux-appimage"; \
	$(BLUE)Creating dir: $$PKG_DIR_NAME$(DONE); \
	mkdir -p "$$PKG_DIR_NAME"; \
	$(BLUE)Moving Marten.AppImage$(DONE); \
	cp -p "Marten.AppImage" "$$PKG_DIR_NAME/Marten.AppImage"; \
	$(BLUE)Creating Portable Home directory$(DONE); \
	mkdir -p "$$PKG_DIR_NAME/Marten.AppImage.home"; \
...
DOCKER_IMAGE_NAME := marten-linux-builder
DOCKER_FLUTTER_VOL := marten-flutter-sdk-cache
DOCKER_PUB_VOL := marten-pub-cache
...
windows-libs:
	...
	if [ -f "$(DESKTOP_OUT)/marten-core.dll" ]; then mv -f "$(DESKTOP_OUT)/marten-core.dll" "$(DESKTOP_OUT)/$(LIB_NAME).dll"; fi
	if [ -f "$(DESKTOP_OUT)/MartenCli.exe" ]; then mv -f "$(DESKTOP_OUT)/MartenCli.exe" "$(DESKTOP_OUT)/$(CLI_NAME).exe"; fi
...
linux-amd64-libs:
	...
	if [ -f "$(DESKTOP_OUT)/lib/marten-core.so" ]; then mv -f "$(DESKTOP_OUT)/lib/marten-core.so" "$(DESKTOP_OUT)/lib/$(LIB_NAME).so"; fi
	if [ -f "$(DESKTOP_OUT)/MartenCli" ]; then mv -f "$(DESKTOP_OUT)/MartenCli" "$(DESKTOP_OUT)/$(CLI_NAME)"; fi
...
macos-libs:
	...
	if [ -f "$(DESKTOP_OUT)/marten-core.dylib" ]; then mv -f "$(DESKTOP_OUT)/marten-core.dylib" "$(DESKTOP_OUT)/$(LIB_NAME).dylib"; fi

ios-libs: #not tested
	mkdir -p $(IOS_OUT)
	rm -rf $(IOS_OUT)/MartenCore.xcframework
...
build-headers:
	make -C marten-core -f Makefile headers && mv $(BINDIR)/$(CORE_NAME)-headers.h $(BINDIR)/marten-core.h

build-android-libs:
	make -C marten-core -f Makefile android
	mv $(BINDIR)/$(LIB_NAME).aar $(ANDROID_OUT)/

build-windows-libs:
	make -C marten-core -f Makefile windows-amd64

build-linux-libs:
	make -C marten-core -f Makefile linux-amd64

build-macos-libs:
	make -C marten-core -f Makefile macos

build-ios-libs:
	rm -rf $(IOS_OUT)/MartenCore.xcframework
	make -C marten-core -f Makefile ios
	mv $(BINDIR)/MartenCore.xcframework $(IOS_OUT)/MartenCore.xcframework
