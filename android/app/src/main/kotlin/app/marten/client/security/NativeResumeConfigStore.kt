...
object NativeResumeConfigStore {
    ...
    fun storeFromPlaintextFile(context: Context, source: File): StoredConfig {
        ...
        try {
            ...
            if (target.exists() && !target.delete()) error("failed to replace encrypted recovery config")
            if (!staged.renameTo(target)) error("failed to promote encrypted recovery config")
        } finally {
            staged.delete()
            plaintext.fill(0)
        }
