...
class PreferencesVersion2Migration extends PreferencesMigrationStep with InfraLogger {
  ...
  @override
  Future<void> migrate() async {
    if (sharedPreferences.getInt("mtu") == 9000) {
      loggy.debug("changing [mtu] from old default [9000] to [1280]");
      await sharedPreferences.setInt("mtu", 1280);
    }
  }
}
...
class PreferencesVersion3Migration extends PreferencesMigrationStep with InfraLogger {
  ...
  @override
  Future<void> migrate() async {
    if (!sharedPreferences.containsKey("mtu") || sharedPreferences.getInt("mtu") == 9000) {
      loggy.debug("changing [mtu] from legacy default to [1280]");
      await sharedPreferences.setInt("mtu", 1280);
    }
  }
}
...
class PreferencesVersion6Migration extends PreferencesMigrationStep with InfraLogger {
  ...
  @override
  Future<void> migrate() async {
    final remoteDnsAddress = sharedPreferences.getString("remote-dns-address");
    if (remoteDnsAddress == "tcp://8.8.8.8" || remoteDnsAddress == "tcp://9.9.9.9") {
      loggy.debug("restoring [remote-dns-address] from TCP default [$remoteDnsAddress] to native [9.9.9.9]");
      await sharedPreferences.setString("remote-dns-address", "9.9.9.9");
    }
  }
}
...
class PreferencesVersion7Migration extends PreferencesMigrationStep with InfraLogger {
  ...
  Future<void> migrate() async {
    final remoteDnsAddress = sharedPreferences.getString("remote-dns-address");
    if (remoteDnsAddress == "9.9.9.9" || remoteDnsAddress == "tcp://8.8.8.8" || remoteDnsAddress == "tcp://9.9.9.9") {
      loggy.debug("restoring [remote-dns-address] from legacy default [$remoteDnsAddress] to native [8.8.8.8]");
      await sharedPreferences.setString("remote-dns-address", "8.8.8.8");
    }
  }
}
