...
import 'package:marten/gen/marten_core_generated_bindings.dart';
...
final _logger = Loggy('MartenCoreFFI');
...
class CoreInterfaceDesktop extends CoreInterface with InfraLogger {
  static final MartenCoreNativeLibrary _box = _gen();

  static MartenCoreNativeLibrary _gen() {
    String fullPath = "";
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      fullPath = "marten-core";
    }
    if (Platform.isWindows) {
      fullPath = p.join(fullPath, "marten-core.dll");
    } else if (Platform.isMacOS) {
      fullPath = p.join(fullPath, "marten-core.dylib");
    } else {
      fullPath = p.join(fullPath, "marten-core.so");
    }

    _logger.debug('marten-core native libs path: "$fullPath"');
    ...
    return MartenCoreNativeLibrary(lib);
  }
