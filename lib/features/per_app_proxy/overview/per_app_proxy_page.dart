...
import 'package:marten/features/per_app_proxy/model/app_package_info.dart';
...
class PerAppProxyPage extends HookConsumerWidget with PresLogger {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final currentMode = mode?.toPerAppProxy() ?? PerAppProxyMode.off;
    ...
    void exitToSettingsRoot() => context.goNamed('settings');

    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) exitToSettingsRoot();
      },
      child: Scaffold(
                          ...
                          MenuItemButton(
                            ...
                            onPressed: () async => await ref
                                ...
                                .then((shouldImport) async {
                                  if (shouldImport) {
                                    await ref.read(PerAppProxyProvider(mode).notifier).importClipboard();
                                  }
                                }),
                          ),
                          MenuItemButton(
                    ...
                    child: ListView(
                      ...
                      children: [
                        PopupMenuButton(
                          ...
                          tooltip: currentMode.present(t).message,
                          ...
                          onSelected: (e) async {
                            ...
                            if (e != PerAppProxyMode.off) {
                              await ref.read(PerAppProxyProvider(e.toAppProxy()).notifier).syncNativeSelection();
                            }
                        ...
                        const Gap(8),
                        ActionChip(
                          avatar: const Icon(FluentIcons.cloud_link_24_regular, size: 18),
                          label: Text(t.pages.settings.routing.perAppProxy.subscriptionBypass.entry),
                          onPressed: () => context.goNamed('subscriptionBypass'),
                        ),
                      ],
                    ),
                  ),
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(Icons.alt_route_rounded, color: theme.colorScheme.onSecondaryContainer),
                    const Gap(12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(
                            '${t.pages.settings.routing.perAppProxy.currentMode}: ${currentMode.present(t).title}',
