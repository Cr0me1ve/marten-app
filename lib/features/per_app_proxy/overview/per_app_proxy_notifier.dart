...
import 'package:marten/features/per_app_proxy/data/auto_selection_repository.dart';
import 'package:marten/features/per_app_proxy/data/auto_selection_repository_provider.dart';
import 'package:marten/features/per_app_proxy/data/selected_data_provider.dart';
import 'package:marten/features/per_app_proxy/model/per_app_proxy_backup.dart';
...
class PerAppProxy extends _$PerAppProxy with AppLogger {
  ...
  Future<bool> shareOnGithub() async {
    ...
    if (userSelected.isNotEmpty || forceDeselected.isNotEmpty) {
      ...
      body = '```\n$body\n```';
      UriUtils.tryLaunch(Uri.parse('https://github.com/Cr0me1ve/Android-GFW-Apps/issues/new?title=$title&body=$body'));
      return true;
    } else {
      ref
          .read(inAppNotificationControllerProvider)
