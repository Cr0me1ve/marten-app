...
import 'package:marten/features/per_app_proxy/overview/per_app_proxy_service_notifier.dart';
import 'package:marten/features/profile/notifier/profiles_update_notifier.dart';
import 'package:marten/features/shortcut/shortcut_wrapper.dart';
import 'package:marten/features/system_tray/notifier/system_tray_notifier.dart';
import 'package:marten/features/window/widget/window_wrapper.dart';
...
class App extends HookConsumerWidget with WidgetsBindingObserver, PresLogger {
  ...
  void onPause(WidgetRef ref) {
    ...
    ref.read(martenCoreServiceProvider).closeFront();
  }

  void onResume(WidgetRef ref) {
    // if (PlatformUtils.isDesktop) return;
    final wasPaused = isOnPauseCalled;
    unawaited(_resumeCoreAndCheckRoute(ref, wasPaused: wasPaused));
  ...
  Future<void> _resumeCoreAndCheckRoute(WidgetRef ref, {required bool wasPaused}) async {
    if (!wasPaused) return;
    await ref.read(martenCoreServiceProvider).init();
    unawaited(ref.read(foregroundProfilesUpdateNotifierProvider.notifier).checkDueNow());
    ref.read(connectionNotifierProvider.notifier).checkRouteAfterResume();
  }
